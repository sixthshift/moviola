	...
	*/
	...
	var Story = class {
		...
		_update() {
			...
			const active = activeIndex(this.steps.map((s) => s.getBoundingClientRect().top), trigger);
			if (active !== this.active) this._activate(active);
			const story = storyProgress(first.getBoundingClientRect().top, last.getBoundingClientRect().bottom, trigger);
			let step = 0;
